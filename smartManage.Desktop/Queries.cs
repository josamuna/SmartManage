...
namespace smartManage.Desktop
{
    class Queries
    {
        private static Queries fact;

        private Queries()
        {
        }

        internal static Queries GetInstance()
        {
            if (fact == null)
                fact = new Queries();
            return fact;
        ...
        internal string CommonQueryMateriel(string criteria)
        {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.Append(@"select materiel.id,materiel.code_str as 'Code',categorie_materiel.designation as 'CatMat',compte.numero as 'Numero',CONVERT(varchar(10),date_acquisition,3) as 'DateAcq',garantie.valeur as 'Garantie',marque.designation as 'Marque',
        ...
        {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.Append(@"select affectation_materiel.id as 'NumAff',affectation_materiel.date_affectation as 'Date',salle.designation as 'Salle',AC.code_str as 'AnneeAcademique',lieu_affectation.designation as 'LieuAffectation',type_lieu_affectation.designation as 'TypeLieuAffectation',
            ...
            left outer join fonction on fonction.id=lieu_affectation.id_fonction").Append(" ").Append(criteria);
            return stringBuilder.ToString();
        } 
    }
}

...
using smartManage.RadiusStudentsModel;
...
{
    public partial class frmDataViewEtudiant : Form, ICallMainForm, ICRUDGeneral
    {
        ...
        smartManage.RadiusStudentsModel.clsConnexion2 connection = new smartManage.RadiusStudentsModel.clsConnexion2();
        ...
        private void cmdValidate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    clsMetier2.GetInstance().Initialize(connection);

                    if (clsMetier2.GetInstance().isConnect())
                    {
            ...
            try
            {
                ...
                switch (tblMain.SelectedIndex)
                {
                    case 1:
                        ...
                        //Set the new ID
                        if (newID_nas == null)
                            newID_nas = clsMetier2.GetInstance().GenerateLastID("nas");
                    ...
                    case 2:
                        ...
                        //Set the new ID
                        if (newID_user == null)
                            newID_user = clsMetier2.GetInstance().GenerateLastID("radcheck");
        ...
        private void DoActualiseDropDown()
        {
            ...
            try
            {
                ...
                {
                    if (smartManage.Desktop.Properties.Settings.Default.strFormModifieSubForm.Equals(FormActualisation.frmGroupUserStudent.ToString()))
                    {
                        cboGroupeUser.DataSource = clsMetier2.GetInstance().getAllClsradgroupcheck_dt();
                        this.setMembersallcbo(cboGroupeUser, "groupname", "groupname");
                    }
        ...
        private void chkSMS_CheckedChanged(object sender, EventArgs e)
        {
            if (chkSMS.Checked)
            {
                ...
                cboPort.DataSource = clsMetier2.GetInstance().GetAllports();
                cboBaud.DataSource = clsMetier2.GetInstance().LoadBaudPorts();
                cboTimeOut.DataSource = clsMetier2.GetInstance().LoadTimeOut();
        ...
        private void cmdConnect_Click(object sender, EventArgs e)
        {
            try
            {
                clsMetier2.GetInstance().OpenConnection(clsMetier2.GetInstance().RecupNumeroPort(cboPort.Text), Convert.ToInt32(cboBaud.SelectedValue.ToString()), Convert.ToInt32(cboTimeOut.SelectedValue.ToString()));
        ...
        private void cmdSendMsg_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if(txtDestinataire.Text.Split(';').Length > 1)
                    //Envoie multiple
                    clsMetier2.GetInstance().SendManySMS(txtMsgSend.Text, txtDestinataire.Text);
                else
                    clsMetier2.GetInstance().SendOneSMS(txtMsgSend.Text, txtDestinataire.Text);
        ...
        private void frmDataViewEtudiant_Load(object sender, EventArgs e)
        {
        ...
        private void frmDataViewEtudiant_Activated(object sender, EventArgs e)
        {
        ...
        private void lblGeneratePassword_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            try
            {
                if (string.IsNullOrEmpty(txtChipherStudent.Text))
                    throw new Exception("Veuiller spécifier la clé de chiffrement svp");
                else
                {
                    //Call chipher
                    txtPasswordUser.Text = DoKey.Crypte(txtNomUser.Text, txtChipherStudent.Text).Substring(0, 4);
                }
            }
            catch(Exception ex)
            {
                MessageBox.Show(string.Format("Echec de la génération du mot de passe, {0}", ex.Message), "Génération mot de passe", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
        }

...
{
    public partial class frmGarantie : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clsgarantie materiel = new clsgarantie();
        int? newID = null;
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("garantie");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clsgarantie> lstItemSearch = new List<clsgarantie>();
                lstItemSearch = clsMetier.GetInstance().getAllClsgarantie(criteria);
        ...
        {
            ((clsgarantie)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clsgarantie)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clsgarantie)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clsgarantie)bdsrc.Current));

...
{
    public partial class frmIntensite : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clsintensite materiel = new clsintensite();
        int? newID = null;
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClsintensite();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("intensite");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clsintensite> lstItemSearch = new List<clsintensite>();
                lstItemSearch = clsMetier.GetInstance().getAllClsintensite(criteria);
        ...
        {
            ((clsintensite)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clsintensite)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clsintensite)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clsintensite)bdsrc.Current));
        ...
        private void frmIntensite_Load(object sender, EventArgs e)
        {

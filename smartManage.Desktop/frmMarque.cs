...
{
    public partial class frmMarque : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clsmarque materiel = new clsmarque();
        int? newID = null;
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClsmarque();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("marque");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clsmarque> lstItemSearch = new List<clsmarque>();
                lstItemSearch = clsMetier.GetInstance().getAllClsmarque(criteria);
        ...
        {
            ((clsmarque)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clsmarque)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clsmarque)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clsmarque)bdsrc.Current));
        ...
        private void frmMarque_Load(object sender, EventArgs e)
        {

...
{
    public partial class frmNbrConsole : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clsconsole materiel = new clsconsole();
        int? newID = null;

        public frmNbrConsole()
        {
            InitializeComponent();
        }
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClsconsole();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("console");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clsconsole> lstItemSearch = new List<clsconsole>();
                lstItemSearch = clsMetier.GetInstance().getAllClsconsole(criteria);
        ...
        {
            ((clsconsole)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clsconsole)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clsconsole)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clsconsole)bdsrc.Current));
        ...
        private void frmNbrConsole_Load(object sender, EventArgs e)
        {

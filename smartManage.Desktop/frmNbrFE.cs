...
{
    public partial class frmNbrFE : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clsfe materiel = new clsfe();
        int? newID = null;
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClsfe();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("fe");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clsfe> lstItemSearch = new List<clsfe>();
                lstItemSearch = clsMetier.GetInstance().getAllClsfe(criteria);
        ...
        {
            ((clsfe)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clsfe)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clsfe)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clsfe)bdsrc.Current));
        ...
        private void frmNbrFE_Load(object sender, EventArgs e)
        {

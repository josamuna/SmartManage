...
{
    public partial class frmNbrFO : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clsfo materiel = new clsfo();
        int? newID = null;
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClsfo();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("fo");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clsfo> lstItemSearch = new List<clsfo>();
                lstItemSearch = clsMetier.GetInstance().getAllClsfo(criteria);
        ...
        {
            ((clsfo)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clsfo)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clsfo)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clsfo)bdsrc.Current));
        ...
        private void frmNbrFO_Load(object sender, EventArgs e)
        {

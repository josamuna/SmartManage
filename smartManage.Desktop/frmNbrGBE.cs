...
{
    public partial class frmNbrGBE : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clsgbe materiel = new clsgbe();
        int? newID = null;
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClsgbe();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("gbe");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clsgbe> lstItemSearch = new List<clsgbe>();
                lstItemSearch = clsMetier.GetInstance().getAllClsgbe(criteria);
        ...
        {
            ((clsgbe)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clsgbe)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clsgbe)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clsgbe)bdsrc.Current));
        ...
        private void frmNbrGBE_Load(object sender, EventArgs e)
        {

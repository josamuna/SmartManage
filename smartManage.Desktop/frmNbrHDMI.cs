...
{
    public partial class frmNbrHDMI : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clshdmi materiel = new clshdmi();
        int? newID = null;
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClshdmi();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("hdmi");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clshdmi> lstItemSearch = new List<clshdmi>();
                lstItemSearch = clsMetier.GetInstance().getAllClshdmi(criteria);
        ...
        {
            ((clshdmi)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clshdmi)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clshdmi)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clshdmi)bdsrc.Current));
        ...
        private void frmNbrHDMI_Load(object sender, EventArgs e)
        {

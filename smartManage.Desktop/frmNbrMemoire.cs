...
{
    public partial class frmNbrMemoire : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clsmemoire materiel = new clsmemoire();
        int? newID = null;

        public frmNbrMemoire()
        {
            InitializeComponent();
        }
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClsmemoire();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("memoire");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clsmemoire> lstItemSearch = new List<clsmemoire>();
                lstItemSearch = clsMetier.GetInstance().getAllClsmemoire(criteria);
        ...
        {
            ((clsmemoire)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clsmemoire)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clsmemoire)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clsmemoire)bdsrc.Current));
        ...
        private void frmNbrMemoire_Load(object sender, EventArgs e)
        {

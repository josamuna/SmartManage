...
{
    public partial class frmNbrSerial : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clsserial materiel = new clsserial();
        int? newID = null;
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClsserial();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("serial");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clsserial> lstItemSearch = new List<clsserial>();
                lstItemSearch = clsMetier.GetInstance().getAllClsserial(criteria);
        ...
        {
            ((clsserial)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clsserial)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clsserial)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clsserial)bdsrc.Current));
        ...
        private void frmNbrSerial_Load(object sender, EventArgs e)
        {

...
{
    public partial class frmNbrSortieAudio : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clssorties_audio materiel = new clssorties_audio();
        int? newID = null;

        public frmNbrSortieAudio()
        {
            InitializeComponent();
        }
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClssorties_audio();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("sorties_audio");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clssorties_audio> lstItemSearch = new List<clssorties_audio>();
                lstItemSearch = clsMetier.GetInstance().getAllClssorties_audio(criteria);
        ...
        {
            ((clssorties_audio)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clssorties_audio)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clssorties_audio)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clssorties_audio)bdsrc.Current));
        ...
        private void frmNbrSortieAudio_Load(object sender, EventArgs e)
        {

...
{
    public partial class frmNbrUSB : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clsusb materiel = new clsusb();
        int? newID = null;
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClsusb();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("usb");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clsusb> lstItemSearch = new List<clsusb>();
                lstItemSearch = clsMetier.GetInstance().getAllClsusb(criteria);
        ...
        {
            ((clsusb)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clsusb)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clsusb)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clsusb)bdsrc.Current));
        ...
        private void frmNbrUSB_Load(object sender, EventArgs e)
        {

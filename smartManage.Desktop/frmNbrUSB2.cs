...
{
    public partial class frmNbrUSB2 : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clsusb2 materiel = new clsusb2();
        int? newID = null;
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClsusb2();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("usb2");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clsusb2> lstItemSearch = new List<clsusb2>();
                lstItemSearch = clsMetier.GetInstance().getAllClsusb2(criteria);
        ...
        {
            ((clsusb2)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clsusb2)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clsusb2)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clsusb2)bdsrc.Current));
        ...
        private void frmNbrUSB2_Load(object sender, EventArgs e)
        {

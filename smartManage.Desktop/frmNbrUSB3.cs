...
{
    public partial class frmNbrUSB3 : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clsusb3 materiel = new clsusb3();
        int? newID = null;
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClsusb3();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("usb3");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clsusb3> lstItemSearch = new List<clsusb3>();
                lstItemSearch = clsMetier.GetInstance().getAllClsusb3(criteria);
        ...
        {
            ((clsusb3)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clsusb3)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clsusb3)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clsusb3)bdsrc.Current));
        ...
        private void frmNbrUSB3_Load(object sender, EventArgs e)
        {

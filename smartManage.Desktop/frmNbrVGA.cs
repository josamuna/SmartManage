...
{
    public partial class frmNbrVGA : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clsvga materiel = new clsvga();
        int? newID = null;
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClsvga();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("vga");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clsvga> lstItemSearch = new List<clsvga>();
                lstItemSearch = clsMetier.GetInstance().getAllClsvga(criteria);
        ...
        {
            ((clsvga)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clsvga)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clsvga)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clsvga)bdsrc.Current));
        ...
        private void frmNbrVGA_Load(object sender, EventArgs e)
        {

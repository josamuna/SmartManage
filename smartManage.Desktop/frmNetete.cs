...
{
    public partial class frmNetete : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clsnetette materiel = new clsnetette();
        int? newID = null;
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClsnetette();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("netette");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clsnetette> lstItemSearch = new List<clsnetette>();
                lstItemSearch = clsMetier.GetInstance().getAllClsnetette(criteria);
        ...
        {
            ((clsnetette)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clsnetette)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clsnetette)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clsnetette)bdsrc.Current));
        ...
        private void frmNetete_Load(object sender, EventArgs e)
        {

...
{
    public partial class frmPoids : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clspoids materiel = new clspoids();
        int? newID = null;
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClspoids();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("poids");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clspoids> lstItemSearch = new List<clspoids>();
                lstItemSearch = clsMetier.GetInstance().getAllClspoids(criteria);
        ...
        {
            ((clspoids)bdsrc.Current).User_modified = Properties.Settings.Default.UserConnected;
            ((clspoids)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clspoids)bdsrc.Current));
            if (record == 0)
                throw new CustomException(stringManager.GetString("StringZeroRecordAffectedMessage", CultureInfo.CurrentUICulture));
            else
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clspoids)bdsrc.Current));
                    if (record == 0)
                        throw new CustomException(stringManager.GetString("StringZeroRecordAffectedMessage", CultureInfo.CurrentUICulture));
                    else
        ...
        private void frmPoids_Load(object sender, EventArgs e)
        {

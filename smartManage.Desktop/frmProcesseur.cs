...
{
    public partial class frmProcesseur : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clsprocesseur materiel = new clsprocesseur();
        int? newID = null;

        public frmProcesseur()
        {
            InitializeComponent();
        }
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClsprocesseur();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("processeur");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clsprocesseur> lstItemSearch = new List<clsprocesseur>();
                lstItemSearch = clsMetier.GetInstance().getAllClsprocesseur(criteria);
        ...
        {
            ((clsprocesseur)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clsprocesseur)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clsprocesseur)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clsprocesseur)bdsrc.Current));
        ...
        private void frmProcesseur_Load(object sender, EventArgs e)
        {

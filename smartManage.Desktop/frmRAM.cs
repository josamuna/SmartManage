...
namespace smartManage.Desktop
{
    public partial class frmRAM : Form
    {
        ...
        private clsram materiel = new clsram();
        int? newID = null;
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClsram();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("ram");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clsram> lstItemSearch = new List<clsram>();
                lstItemSearch = clsMetier.GetInstance().getAllClsram(criteria);
        ...
        {
            ((clsram)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clsram)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clsram)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clsram)bdsrc.Current));

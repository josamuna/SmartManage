...
{
    public partial class frmTailleEcran : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clstaille_ecran materiel = new clstaille_ecran();
        int? newID = null;

        public frmTailleEcran()
        {
            InitializeComponent();
        }
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClstaille_ecran();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("taille_ecran");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clstaille_ecran> lstItemSearch = new List<clstaille_ecran>();
                lstItemSearch = clsMetier.GetInstance().getAllClstaille_ecran(criteria);
        ...
        {
            ((clstaille_ecran)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clstaille_ecran)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clstaille_ecran)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clstaille_ecran)bdsrc.Current));
        ...
        private void frmTailleEcran_Load(object sender, EventArgs e)
        {

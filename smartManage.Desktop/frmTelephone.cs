...
namespace smartManage.Desktop
{
    public partial class frmTelephone : Form
    {
        ...
        private clstelephone materiel = new clstelephone();
        int? newID = null;
        ...
        private void BindingCls()
        {
            ...
            SetBindingControls(cboCodePays, "SelectedValue", materiel, "Code");
        ...
        private void BindingList()
        {
            ...
            SetBindingControls(cboCodePays, "SelectedValue", bdsrc, "Code");
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClstelephone(smartManage.Desktop.Properties.Settings.Default.IdentifiantPersonne);
            this.SetDataSource(bdsrc);
            dgv.DataSource = bdsrc;

            Dictionary<string, string> dicoCodePays = new Dictionary<string, string>();
            dicoCodePays.Add("+243", "RDC (+243)");
            dicoCodePays.Add("+250", "Rwanda (+250)");
            dicoCodePays.Add("+242", "Congo Brazaville (+242)");

            cboCodePays.DataSource = new BindingSource(dicoCodePays, null);
            this.setMembersallcbo(cboCodePays, "Value", "Key");
            cboPersonne.DataSource = clsMetier.GetInstance().getAllClspersonne_unique(smartManage.Desktop.Properties.Settings.Default.IdentifiantPersonne);
            this.setMembersallcbo(cboPersonne, "NomComplet", "Id");

            List<ComboBox> lstCombo = new List<ComboBox>() { cboCodePays };
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("telephone");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clstelephone> lstItemSearch = new List<clstelephone>();
                lstItemSearch = clsMetier.GetInstance().getAllClstelephone(criteria, smartManage.Desktop.Properties.Settings.Default.IdentifiantPersonne);
        ...
        {
            ((clstelephone)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clstelephone)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clstelephone)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clstelephone)bdsrc.Current));

...
{
    public partial class frmTensionAdaptateur : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clstension_adaptateur materiel = new clstension_adaptateur();
        int? newID = null;

        public frmTensionAdaptateur()
        {
            InitializeComponent();
        }
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClstension_adaptateur();
        ...
        private void New()
        {
            ...
            materiel = new clstension_adaptateur();
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("tension_adaptateur");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clstension_adaptateur> lstItemSearch = new List<clstension_adaptateur>();
                lstItemSearch = clsMetier.GetInstance().getAllClstension_adaptateur(criteria);
        ...
        {
            ((clstension_adaptateur)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clstension_adaptateur)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clstension_adaptateur)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clstension_adaptateur)bdsrc.Current));
        ...
        private void frmTensionAdaptateur_Load(object sender, EventArgs e)
        {

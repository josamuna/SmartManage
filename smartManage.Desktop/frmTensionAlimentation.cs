...
{
    public partial class frmTensionAlimentation : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clstension_alimentation materiel = new clstension_alimentation();
        int? newID = null;

        public frmTensionAlimentation()
        {
            InitializeComponent();
        }
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClstension_alimentation();
        ...
        private void New()
        {
            ...
            materiel = new clstension_alimentation();
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("tension_alimentation");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clstension_alimentation> lstItemSearch = new List<clstension_alimentation>();
                lstItemSearch = clsMetier.GetInstance().getAllClstension_alimentation(criteria);
        ...
        {
            ((clstension_alimentation)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clstension_alimentation)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clstension_alimentation)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clstension_alimentation)bdsrc.Current));
        ...
        private void frmTensionAlimentation_Load(object sender, EventArgs e)
        {

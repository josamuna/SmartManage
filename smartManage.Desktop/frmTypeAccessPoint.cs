...
{
    public partial class frmTypeAccessPoint : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clstype_AP materiel = new clstype_AP();
        int? newID = null;

        public frmTypeAccessPoint()
        {
            InitializeComponent();
        }
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClstype_AP();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("type_AP");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clstype_AP> lstItemSearch = new List<clstype_AP>();
                lstItemSearch = clsMetier.GetInstance().getAllClstype_AP(criteria);
        ...
        {
            ((clstype_AP)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clstype_AP)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clstype_AP)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clstype_AP)bdsrc.Current));
        ...
        private void frmTypeAccessPoint_Load(object sender, EventArgs e)
        {

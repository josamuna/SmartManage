...
{
    public partial class frmTypeOS : Form
    {
        BindingSource bdsrc = new BindingSource();
        bool blnModifie = false;
        private clstype_OS materiel = new clstype_OS();
        int? newID = null;
        ...
        {
            bdsrc.DataSource = clsMetier.GetInstance().getAllClstype_OS();
        ...
        private void New()
        {
            ...
            //Set the new ID
            if (newID == null)
                newID = clsMetier.GetInstance().GenerateLastID("type_OS");
        ...
        private void Search(string criteria)
        {
            ...
            {
                List<clstype_OS> lstItemSearch = new List<clstype_OS>();
                lstItemSearch = clsMetier.GetInstance().getAllClstype_OS(criteria);
        ...
        {
            ((clstype_OS)bdsrc.Current).User_modified = smartManage.Desktop.Properties.Settings.Default.UserConnected;
            ((clstype_OS)bdsrc.Current).Date_modified = DateTime.Now;

            int record = materiel.update(((clstype_OS)bdsrc.Current));
        ...
        private void Delete()
        {
            if (blnModifie)
            {
                ...
                {
                    record = materiel.delete(((clstype_OS)bdsrc.Current));

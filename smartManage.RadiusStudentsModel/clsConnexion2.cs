...
{
    public class clsConnexion2 : INotifyPropertyChanged, IDataErrorInfo
    {
        private string server = @"localhost", db, user = "root", pwd = "root";
        private List<clsConnexion2> lstbd = new List<clsConnexion2>();
        public event PropertyChangedEventHandler PropertyChanged;
        private Dictionary<string, string> _errorInfos;
        public void changeConnect()
        {
            clsMetier2.GetInstance().setDB(db);
        }
        ...
        public clsConnexion2()
        {
            _errorInfos = new Dictionary<string, string>();
        }
        public bool isConnexion()
        {
            clsMetier2.GetInstance().Initialize(this, 1);
            return clsMetier2.GetInstance().isConnect();
        }
        public override string ToString()
        {
            return string.Format("{0}", this.server);
        }
        public clsConnexion2(string server, string db, string user, string pwd)
        {
            _errorInfos = new Dictionary<string, string>();
            this.server = server;

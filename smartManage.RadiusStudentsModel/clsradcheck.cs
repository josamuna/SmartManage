...
namespace smartManage.RadiusStudentsModel
{
    public class clsradcheck:clsradusergroup
    {
        ...
        {
            return clsMetier2.GetInstance().getAllClsradcheck();
        }
        public new List<clsradcheck> listes(string criteria)
        {
            return clsMetier2.GetInstance().getAllClsradcheck(criteria);
        }
        public new int inserts()
        {
            return clsMetier2.GetInstance().insertClsradcheck(this);
        }
        public int inserts(DataRowView varscls)
        {
            return clsMetier2.GetInstance().insertClsradcheck_dtrowv(varscls);
        }
        public int update(DataRowView varscls)
        {
            return clsMetier2.GetInstance().updateClsradcheck_dtrowv(varscls);
        }
        public new int update()
        {
            return clsMetier2.GetInstance().updateClsradcheck(this);
        }
        public int delete(DataRowView varscls)
        {
            return clsMetier2.GetInstance().deleteClsradcheck_dtrowv(varscls);
        }
        public new int delete()
        {
            return clsMetier2.GetInstance().deleteClsradcheck(this);
        }
